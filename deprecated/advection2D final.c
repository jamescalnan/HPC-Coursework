...
int main(){
  ...
  // x2 and y2 are private by default because theyre declared inside the loop, so no need for scope changes
  ...
  /* LOOP 10 */
  // Cannot parallelise this loop as it would jumble the output order
  for (int i=0; i<NX+2; i++){
    for (int j=0; j<NY+2; j++){
